...
data _—→_ : ∀ {Γ A} → (Γ ⊢ A) → (Γ ⊢ A) → Set where
  ...
  β-[] : ∀ {Γ A B} {L : Γ ⊢ `List A} {M : Γ ⊢ B} {N : Γ , A , `List A ⊢ B}
    → Value L
      -----------------------------
    → caseL `[] [[]⇒ M |∷⇒ N ] —→ M

  β-∷ : ∀ {Γ A B} {L : Γ ⊢ A} {Ls : Γ ⊢ `List A} {M : Γ ⊢ B} {N : Γ , A , `List A ⊢ B}
    → Value L
    → Value Ls
      -----------------------------------------------
...
swap⊎ : ∀ {A B} → ∅ ⊢ A `⊎ B ⇒ B `⊎ A
swap⊎ = ƛ case⊎ (# 0) [inj₁⇒ `inj₂ (# 0) |inj₂⇒ `inj₁ (# 0) ]
...
from×⊤-case : ∀ {A} → ∅ ⊢ A `× `⊤ ⇒ A
from×⊤-case = ƛ case× (# 0) (case⊤ (# 0) [tt⇒ (# 1) ])

to⊎⊥ : ∀ {A} → ∅ ⊢ A ⇒ A `⊎ `⊥
to⊎⊥ = ƛ (`inj₁ (# 0))

from⊎⊥ : ∀ {A} → ∅ ⊢ A `⊎ `⊥ ⇒ A
from⊎⊥ = ƛ case⊎ (# 0) [inj₁⇒ # 0 |inj₂⇒ case⊥ (# 0) [] ]
